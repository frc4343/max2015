 ...
 *
 */
public class TurnWithGyro4 extends Command {
	 ...
	 */
    public TurnWithGyro4(double speed, double angleToTurnTo, boolean resetGyro) {
    ...
    protected void execute() {
    	...
    	if (stage1) {
    		if (angleToTurnTo > 0 && !(Robot.driveTrain.getHeading() >= angleToTurnTo)) { // if angle is positive
            	Robot.driveTrain.tankDrive(0.5, -0.5); // robot turns right 
            } else if (angleToTurnTo < 0 && !(Robot.driveTrain.getHeading() <= angleToTurnTo)){ // angle is negative
            	Robot.driveTrain.tankDrive(-0.5, 0.5); // robot turns left
            } else { // when angle is achieved
            	Robot.driveTrain.tankDrive(0, 0); // full stop
            	stage2 = true;
            }
